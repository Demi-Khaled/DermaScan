  ...
  void initState() {
    ...
    if (result != null && widget.lesion == null) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        try {
           final newLesion = await Provider.of<LesionService>(context, listen: false)
               .createLesion('New Lesion', 'Unknown', '', initialScan: result);
           if (mounted) setState(() { _lesion = newLesion; });
        } catch (e) {
            // handle
        } finally {
  ...
  void _deleteLesion() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'This requires a Delete endpoint to be added to the backend API first, but normally this would permanently remove this lesion.',
        ),
        actions: [
          TextButton(
